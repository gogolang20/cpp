...
// avl tree 是否有某个key的node
node *contain(node *root, int key) {
...
// 比 key 大的最小node, 包含等于
node *findRightKey(node *root, int key) {
    if (root == nullptr) return nullptr;

    if (root->data == key) {
        return root;
    } else if (root->data < key) {
        return findRightKey(root->right, key);
    } else {
        node* ans = findRightKey(root->left, key);
        return ans == nullptr ? root->left : ans;
    }
}

// node *findLeftKey(node *root, int key) {
//     if (root == nullptr) return nullptr;

//     if (root->data == key) {
//         return root;
//     } else if (root->data > key) {
//         return findLeftKey(root->left, key);
//     } else {
//         node* ans = findLeftKey(root->right, key);
//         return ans == nullptr ? root->right : ans;
//     }
// }

// 比 key 大的最小node, 不包含等于
node *findRightSideKey(node *root, int key) {
    if (root == nullptr) return nullptr;

   if (root->data <= key) {
        return findRightSideKey(root->right, key);
    } else {
        node* ans = findRightSideKey(root->left, key);
        return ans == nullptr ? root : ans;
    }
}

// node *findLeftSideKey(node *root, int key) {
//     if (root == nullptr) return nullptr;

//    if (root->data >= key) {
//         return findLeftSideKey(root->left, key);
//     } else {
//         node* ans = findLeftSideKey(root->right, key);
//         return ans == nullptr ? root : ans;
//     }
// }
